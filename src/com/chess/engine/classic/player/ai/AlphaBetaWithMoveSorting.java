...
public class AlphaBetaWithMoveSorting extends Observable implements MoveStrategy {
    ...
    private enum MoveSorter {
        ...
        SORT {
            @Override
            Collection<Move> sort(final Board board, final Collection<Move> moves, final Move bestMove) {
        ...
        CAPTURE_SORT{
            ...
            private Collection<Move> getCaptureMoves(final Board board, final Collection<Move> moves) {
        ...
        private static Collection<Move> getSortedMoves(final Board board, final Collection<Move> moves, final Move bestMove){
        ...
        abstract Collection<Move> sort(Board board, Collection<Move> moves, Move bestMove);
    }
    ...
    public Move execute(final Board board) {
        List<Move> sortedMoves = (List<Move>) this.moveSorter.sort(board, board.currentPlayer().getLegalMoves(), null);
        ...
        do {
            ...
            sortedMoves = (List<Move>) this.moveSorter.sort(board, board.currentPlayer().getLegalMoves(), bestMove);
    ...
    private Move findBestMoveAlphaBetaMinMax(final Board board, final List<Move> sortedMoves, final int depth){
        ...
        for (final Move move : sortedMoves) {
            ...
            final long candidateMoveDuration = (System.nanoTime() - candidateMoveStartTime) / 1000000;

            System.out.printf("(Depth %d) move: (%d/%d) %s, best: %s [evaluation: %.2f], time: %d ms\n",
                    depth, moveCounter, numMoves, move, bestMove,
                    (double) evaluation / 100,
                    candidateMoveDuration);
            moveCounter++;
        }
    ...
    private int findBestMoveAlphaBetaMinMaxOnlyCaptures(final Board board, final Collection<Move> moves, final int maxDepth) {
        final List<Move> captureMoves = (List<Move>) this.captureSorter.sort(board, moves, null);
        final Player currentPlayer = board.currentPlayer();
        final Alliance alliance = currentPlayer.getAlliance();
        int bestValue = 0;
        int highestSeenValue = Integer.MIN_VALUE;
    ...
    private int executeOnlyCaptures(final Board board){
        ...
        final long startTime = System.currentTimeMillis();
        List<Move> sortedMoves = (List<Move>) this.captureSorter.sort(board, board.currentPlayer().getLegalMoves(), null);
    ...
    public int maxCaptures(final Board board,
                            ...
                            final int highest,
                            final int lowest) {
        final List<Move> captureMoves = (List<Move>) this.captureSorter.sort(board, board.currentPlayer().getLegalMoves(), null);
